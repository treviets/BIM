...
public class ProjectBaseLineController {
	...
	@RequestMapping(value = "/list/baseline/{projectId}/{type}/{clientId}", method = { RequestMethod.GET })
	public Object getBaseLinebyType(@PathVariable("projectId") int projectId, @PathVariable("type") int type, @PathVariable("clientId") int clientId) {
		return projectBaseLineService.getBaseLineByType(projectId, type, clientId);
	}
}
