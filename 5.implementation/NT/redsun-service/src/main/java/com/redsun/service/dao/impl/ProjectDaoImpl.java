...
public class ProjectDaoImpl implements ProjectDao {
	...
	static private String SELECT_PROJECT_BY_RESOURCE = "SELECT p.id, p.code, p.name, p.location, p.description, p.parent_id, p.client_id," + 
			" p.start_date, p.amount, count(*) over() as total_count FROM projects p LEFT JOIN project_resources pr on p.id = pr.project_id " + 
	...
	@Override
	public List<Project> getProjectByResourceId(String resourceCode){
		List<Project> listProjects = null;
		try {
			listProjects = jdbcTemplate.query(SELECT_PROJECT_BY_RESOURCE, new Object[] { resourceCode }, new ProjectMapper());
		} catch (Exception e) {
			log.error(e.getMessage());
		}
		return listProjects;
