...
;( function() {
    ...
    TreeNode.prototype.buildNodeFromText = function (node) {
        ...
        if (this.image) {
            ...
            image.src = this.image.split('*')[0];
            node.appendChild(image);
            
        	var btnRemove = document.createElement('span');
        	btnRemove.id=  "id_" + this.image.split('*')[1];
        	btnRemove.className ="glyphicon glyphicon-remove department-icon-remove";
        	btnRemove.title="edit new department ";
        	btnRemove.onclick=function(){
        		confirm("Are your sure delete the department?")
        		angular.element('#btn-Add').triggerHandler('click');	
        	}
        	node.appendChild(btnRemove);
            var btn = document.createElement('span');
        	//btn.type='button';
        	//btn.appendChild(document.createTextNode("Add Child"));
        	btn.id=  this.image.split('*')[1];
        	btn.className ="glyphicon glyphicon-user department-icon";
        	btn.title="add new department";
        	btn.onclick=function(){alert(this.id)
        		angular.element('#btn-Add').triggerHandler('click');	
        	}
        	node.appendChild(btn);
        	var btnEdit = document.createElement('span');
        	btnEdit.id=  "id_" + this.image.split('*')[1];
        	btnEdit.className ="glyphicon glyphicon-edit department-icon";
        	btnEdit.title="edit new department ";
        	btnEdit.onclick=function(){alert(this.id)
        		angular.element('#btn-Add').triggerHandler('click');	
        	}
